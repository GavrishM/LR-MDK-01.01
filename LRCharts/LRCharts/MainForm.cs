...
namespace LRCharts
{
    public partial class MainForm : Form
    {
        ...
        private void ItemsList_SelectedIndexChanged(object sender, System.EventArgs e)
        {
            ...
            double percent = Math.Round(presenter_.GetProfitPercentByItem(selectedItem), 2);

        }
    }
}
//Вариант 1: Продажи напитков в кафе
//Есть история продаж напитков по дням. В ListBox отображается список напитков (кофе, чай, сок, лимонад и т.д.), по которым были продажи. При выборе напитка строится график (CartesianChart) количества проданных единиц по дням. Цена за единицу напитка фиксированная. На круговой диаграмме (PieChart) отображается вклад (%) каждого напитка в общей выручке кафе.
