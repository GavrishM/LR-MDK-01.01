...
namespace LR2_V7_Gavrish_M
{
    internal class Program
    {
        static void Main(string[] args)
        {
            ...
            //Вывод
            {
                string userQuery = Input.UserQuery(categories);
                Dictionary<string, List<double>> findProductList = Calc.FindProductsList(userQuery, categories);
                Dictionary<string, double> findAveragePrice = Calc.FindAveragePrice(findProductList);
                Dictionary<string, double> sortList = Calc.SortList(findAveragePrice);
                Output.List(sortList);
            }
